#  --- INFOS PARA O SIMULADOR ---
"""
--- Author: Diego L A Abreu
--- Instagram: @_diego.abreu_
...
def show_arena():
    ...
    print("-" * total_width)
    print(f"Tempo de duração: {(time.time() - simulation['time_arena']):.4f}")

    time.sleep(simulation["timer"])


"""
--- CÓDIGO PARA SPIKE PRIME ---
--- O código que vai para o spike prime está abaixo dessa linha ---
"""
import collections
...
DIRECTIONS = {tuple(v): k for k, v in MOVEMENTS.items()}
...
def add_obstracle():
    ...
    pieces[OBSTACLE].append([new_r, new_c])
    arena[new_r][new_c] = OBSTACLE
...
def find_closest_unexplored():
    ...
    while queue:
        ...
        if arena[current_r][current_c] == NON_EXPLORED:
            # --- 2. Validação do Caminho com A* (o ponto crucial) ---

            # Aqui, você precisa garantir que o A* existe e está no escopo.
            path_to_unexplored = a_stars(current_pos)
            if path_to_unexplored:
                return current_pos

            # Se o A* falhar, o BFS continua a busca, ignorando este alvo.
            # O alvo é inalcançável, mas como o BFS continua, ele irá para o segundo "?" mais próximo.
        ...
        for r, c in check_cells:
            ...
            if 0 <= r <= max_r and 0 <= c <= max_c:
                ...
                if arena[r][c] != OBSTACLE:
                    # FILTRO A*: Só retorna se A* encontrar o caminho!
                    if a_stars(target_pos):
                        return target_pos
...
def find_and_expand_closest_boundary():
    ...
    # lista de direções de bordas a verificar
    avaliable_directions = []

    # 1. Filtrando as direções bloqueadas
    for movement in MOVEMENTS.keys():
        # Se a borda ainda não foi selada.
        if not BOUNDARIES_SET[movement]:
            avaliable_directions.append(
                movement
            )  # Adiciona a direção da borda na lista.

    if not avaliable_directions:
        return None

    # Para cada borda que ainda tem a explorar, encontrar a celula livre mais próxima na direção.
    available_edges = []

    for movement in avaliable_directions:
        target_pos = get_closest_edge_target(pieces[ROBO], movement)
        if target_pos:
            dist = heuristics(pieces[ROBO], target_pos)
            available_edges.append((dist, movement, target_pos))
...
def update_agent(zone):
    ...
    if not (0 <= new_pos[0] < len(arena) and 0 <= new_pos[1] < len(arena[0])):
        expand_arena()
...
def main_loop():
    global hasCubes, direction, simulation
    simulation["time_arena"] = time.time()
    while True:
        # show_arena()
    ...
    simulation["time_arena"] = time.time() - simulation["time_arena"]
...
if __name__ == "__main__":
    ...
    for i in range(1, repeat + 1):
        clear_screen()
        reset_simulation()
        bot.arena = bot.generate_random_grid(i)
        # show_arena()
        main_loop()
        simulation["time_mediumn"] = float(
            float(simulation["time_mediumn"] + simulation["time_arena"]) / float(i + 1)
        )
        print(
            f"Run [{i}/{repeat} arenas | {(time.time() - simulation['duration']):.4f} segundos."
        )
    print(
        f"O tempo médio de execução das arenas de: {(simulation['time_mediumn']):.10f} segundos."
    )
